...
function _S(obj) {
	if (_E(obj)) { return ""} else { return obj};
}
...
function isAPIKeyValid(apikey, cb) {
	...
	if (apikey == undefined ) {
		...
		if (cb) { cb(new Error('Invalid apikey'), null); };
	};
	...
	try {
		...
		LogUserApp.findOne(query, function(err, logUserApp){
			...
			if (_E(logUserApp)) {
				if (cb) { cb(new Error('No record found'), null); };
				return;
			};

			if (cb) { cb(null, logUserApp); };
		...
		if (cb) { cb(new Error(e.message), null); };
	...
	try {
		...
		if (match){
			...
			LogUser.update({_id: logUser._id}, {$currentDate: { last_login: true }}, {multi: false} , function(err, logUserUpdated) {
				...
				var authToken = jwt.sign({ _id: logUser._id, username: logUser.username, tag: 'mobilelogger' }, '00MasecR3t', {expiresInMinutes: 2 * 3600});
				...
				var userData = { username: logUser.username,
								 _id: logUser._id,
								 authToken: authToken,
			...
			if (cb) {cb(new Error('Password not match', null))};
		}
		...
		var logUser = new LogUser({
			...
			logger.debug('createUser: ', logUser);
				...
				logUser.save(function(err, logUser) {
					...
					LogGroup.findOneAndUpdate({name: 'ooma'}, {ownerid: logUser._id, $push: {members:logUser._id}}, {upsert: true}, function(err, logGroup){
						if (err) {
...
function saveSessionForValidToken(req,res,cb) {
    ...
    new_loginSession.save(new_loginSession, function(err, session) {
        ...
        var authToken = jwt.sign({ _id: session._id, apikey: loginSession['apikey'], tag: 'mobilelogger','token_access':1,'devid':loginSession['devid']}, '00MasecR3t', {expiresInMinutes: 1 });
    ...
    LogAuthLim.findOne(validation_query,function(err, limitation){
        ...
        if ((!limitation && loginSession['remote_ip'] != undefined )|| limitation['doc_limit'] < nonauth_limitation)
        {
            ...
            new_loginSession.save(new_loginSession, function(err, session) {
                ...
                var authToken = jwt.sign({ _id: session._id, apikey: loginSession['apikey'], tag: 'mobilelogger','token_access':0,'devid':loginSession['devid']}, '00MasecR3t', {expiresInMinutes: 1 });
                res.status(201).json({status: 'success', data: {_id: authToken}});
            })
        }
        else
		...
		jwt.verify(sessionid, '00MasecR3t', function(err, decoded) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				logData.save(function(err, log) {
                    ...
                    {
                        LogAuthLim.findOneAndUpdate({remote_ip: remote_ip},{$inc:{doc_limit:1},devid:devid},function(err,res_cb){
                            if (err) {
                                logger.error('saveLog exception occured: ', err , 'inc data: ', req.body);
                        }})
                    }
					...
					try {
                        ...
                        for (var j = 0; j < event_data.length; j++) {
                             try {
                                 ...
                                 var remote_number;
                                 var call_direction;
                                 event_data_array.push({label: getLabelName(event_data[j].z),
                                 ...
                                 if (data[i].t == 30010){ // for RT_CALL_END
                                     ...
                                     if (getLabelName(event_data[j].z) == 'Remote number'){
                                         remote_number=event_data[j].k; event_data_array.pop() }
                                     if (getLabelName(event_data[j].z) == 'Call duration'){
                                         call_duration=event_data[j].k; event_data_array.pop() }
                                     if (getLabelName(event_data[j].z) == 'Call direction'){
                                         call_direction=decodeEventValue(event_data[j].z,event_data[j].k); event_data_array.pop() }
                                 }
                                 else{
	...
	try {
		...
		LogLogData.count(query, function(err, count) {
				...
				if ( page != undefined && page == "last") {
					...
					if(_E(page) || parseInt(page,10) < 0) {page = 0;}

					skip = parseInt(page,10) * limit;
					if (skip >= count) { skip = count - limit;};
				}
